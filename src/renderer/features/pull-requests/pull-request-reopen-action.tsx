...
export function PullRequestReopenAction({ item }: { item: PullRequestSummary }) {
  ...
  const mutation = trpc.pullRequests.reopen.useMutation({
    onSuccess: async () => {
      setOpen(false)
      setComment("")
  ...
  const errorMessage = mutation.error
    ? isForbidden
      ? "You don't have permission to reopen this pull request."
      : mutation.error.message || "Unable to reopen this pull request. Try again."
    : null
  ...
  return (
    <>
      <Button
        ...
        aria-label="Reopen pull request"
        ...
        >
          ...
          >
            <CanvasDialogBody className="space-y-2">
              <Label htmlFor="pr-reopen-comment">Comment (optional)</Label>
              <Textarea
                id="pr-reopen-comment"
                autoFocus
                value={comment}
